		...
		if (forcePatch || hasDynamicProps || patchFlag !== -1) {
			...
			if (props) {
				if (forcePatch || hasDynamicProps || !optimized || patchFlag & 48) {
					...
					for (const key in props) if (forcePatch && (key.endsWith("value") || key === "indeterminate") || (0, _vue_shared.isOn)(key) && !(0, _vue_shared.isReservedProp)(key) || key[0] === "." || isCustomElement && !(0, _vue_shared.isReservedProp)(key) || dynamicProps && dynamicProps.includes(key)) patchProp(el, key, null, props[key], namespace, parentComponent);

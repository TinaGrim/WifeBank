import { Ai as onUnmounted, Dn as Fragment, Do as toDisplayString, Ei as onMounted, Eo as normalizeStyle, Fa as watch, Jn as computed, Ki as renderList, Mi as openBlock, Qn as createBlock, Ri as provide, Ur as inject, Yi as resolveComponent, Zn as createBaseVNode, co as reactive, cr as createTextVNode, er as createCommentVNode, hi as mergeProps, pr as defineComponent, qi as renderSlot, tr as createElementBlock, uo as ref, wo as normalizeClass, za as withCtx } from "./vue.runtime.esm-bundler-Byn5HWGB.js";
//#region node_modules/vue3-charts/dist/vue3-charts.esm.js
...
function _reduce(fn, acc, list) {
	if (typeof fn === "function") fn = _xwrap(fn);
	if (_isArrayLike(list)) return _arrayReduce(fn, acc, list);
...
var map$1 = /*#__PURE__*/ _curry2(/*#__PURE__*/ _dispatchable(["fantasy-land/map", "map"], _xmap, function map(fn, functor) {
	switch (Object.prototype.toString.call(functor)) {
		...
		case "[object Object]": return _reduce(function(acc, key) {
...
function _equals(a, b, stackA, stackB) {
	if (_objectIs$1(a, b)) return true;
	var typeA = type(a);
	...
	switch (typeA) {
		...
		case "Number":
		case "String":
			if (!(typeof a === typeof b && _objectIs$1(a.valueOf(), b.valueOf()))) return false;
			break;
		case "Date":
			if (!_objectIs$1(a.valueOf(), b.valueOf())) return false;
			break;
...
var getCol = curry$1((col, df) => {
	if (is$1(Number)) return map$2((v) => nth$1(col, v) || "", df).filter((x) => x !== "");
	return map$2(prop$1(col), df);
});
var mapKeys = (fn, data) => zipObj$1(map$2(fn, keys(data)), values$1(data));
var kebabize = (str) => {
...
function transformer$1() {
	var domain = unit, range = unit, interpolate$1 = interpolate, transform, untransform, unknown, clamp = identity$3, piecewise, output, input;
	function rescale() {
...
function formatLocale(locale) {
	var group = locale.grouping === void 0 || locale.thousands === void 0 ? identity$2 : formatGroup(map.call(locale.grouping, Number), locale.thousands + ""), currencyPrefix = locale.currency === void 0 ? "" : locale.currency[0] + "", currencySuffix = locale.currency === void 0 ? "" : locale.currency[1] + "", decimal = locale.decimal === void 0 ? "." : locale.decimal + "", numerals = locale.numerals === void 0 ? identity$2 : formatNumerals(map.call(locale.numerals, String)), percent = locale.percent === void 0 ? "%" : locale.percent + "", minus = locale.minus === void 0 ? "−" : locale.minus + "", nan = locale.nan === void 0 ? "NaN" : locale.nan + "";
...
var Scale = class {
	...
	bandwidth() {
		if (this.type === "band") return this.scale.bandwidth();
		return 0;
	}
	updateRange(range) {
	...
	mapOne(val) {
		...
		if (type === "band") return this.scale(val) + this.scale.bandwidth() / 2;
		return this.scale(val);
	}
	ticks() {
		const { type } = this.config;
		if (this.config.tickValues !== void 0) return this.config.tickValues;
		if (type === "band") return this.scale.domain();
...
var Path = class {
	...
	arc(x, y, r, a0, a1, ccw) {
		...
		else if (da > epsilon$1) this._append`A${r},${r},0,${+(da >= pi)},${cw},${this._x1 = x + r * Math.cos(a1)},${this._y1 = y + r * Math.sin(a1)}`;
	}
	rect(x, y, w, h) {
...
function arc() {
	var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant$2(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null, path = withPath(arc);
	function arc() {
...
function line(x$1, y$1) {
	var defined = constant$2(true), context = null, curve = curveLinear, output = null, path = withPath(line);
...
function area(x0, y0, y1) {
	var x1 = null, defined = constant$2(true), context = null, curve = curveLinear, output = null, path = withPath(area);
...
function pie() {
	...
	function pie(data) {
		var i, n = (data = array$1(data)).length, j, k, sum = 0, index = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau$1, Math.max(-tau$1, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
...
function stack() {
	var keys = constant$2([]), order = stackOrderNone, offset = none$1, value = stackValue;
	function stack(data) {
		var sz = Array.from(keys.apply(this, arguments), stackSeries), i, n = sz.length, j = -1, oz;
		for (const d of data) for (i = 0, ++j; i < n; ++i) (sz[i][j] = [0, +value(d, sz[i].key, j, data)]).data = d;
		for (i = 0, oz = array$1(order(sz)); i < n; ++i) sz[oz[i]].index = i;
		offset(sz, oz);
		return sz;
	}
...
var Chart = class {
	...
	get canvas() {
		...
		if (this.config.axis.primary.hide) axisSpace.y = 0;
		if (this.config.axis.secondary.hide) axisSpace.x = 0;
		return {
	...
	getLayers(type) {
		if (type) return this.layers.filter((l) => l.type === type);
		return this.layers;
	}
	getData(keys) {
...
var usePoints = (dataKeys, props = {
	...
	function getPoints(key, values) {
		...
		if (chart.config.direction === "horizontal") return values.map((val) => {
			return {
		...
		else if (chart.config.direction === "vertical") return values.map((val) => {
			return {
	...
	function update() {
		...
		const data = chart.getStackedData(stacked ? keys : [dataKeys[1]]).find((s) => s.key === secondaryKey);
		if (data) points.value = getPoints(primaryKey, data);
	}
	watch(chart.updates, () => {
		update();
	}, { immediate: true });
...
var useBars = (dataKeys, props = {
	...
	function scales() {
		const { primary, secondary } = chart.scales;
		return {
			bandScale: primary.type === "band" ? primary : secondary,
			linearScale: primary.type === "band" ? secondary : primary
		};
	}
	function getIndex() {
		if (stacked) return 0;
		const idx = chart.getLayers(type).findIndex((l) => l.dataKeys.join(",") === dataKeys.join(","));
		return idx > -1 ? idx : 0;
	}
	...
	function getDiffWidth() {
		if (stacked) return 0;
		const { bandScale } = scales();
		return (bandScale.bandwidth() - gap - chart.getLayers(type).length * getBarWidth()) / 2;
	}
	...
	function update() {
		...
		const [primaryKey, secondaryKey] = dataKeys;
		const data = chart.getStackedData(keys).find((s) => s.key === secondaryKey);
		if (data) bars.value = getBars(primaryKey, data);
	}
	watch(chart.updates, () => {
		update();
	});
...
var useTooltip = () => {
	...
	watch(mouse, () => {
		...
		if (x) if (chart.config.direction === "horizontal") position.value = {
			x,
			y: mouse.position.y
		};
		else if (chart.config.direction === "vertical") position.value = {
			x: mouse.position.x,
			y: x
		};
		else position.value = {
		...
		if (d) payload.value = Object.assign(Object.assign({}, d), { idx: mouse.index });
	});
	return {
		position,
		payload,
	...
	setup(props) {
		...
		function drawAxis() {
			if (chart.data.length > 0) {
				...
				if (config.format) ax.tickFormat(config.format);
				if (config.ticks !== void 0) ax.ticks(config.ticks);
				if (config.tickValues !== void 0) ax.tickValues(config.tickValues);
				if (config.useConfig) config.useConfig(ax);
				const res = select(el.value).call(ax);
				if (props.position === "bottom" && props.rotate) res.selectAll("text").attr("y", 0).attr("x", 9).attr("dy", ".35em").attr("transform", "rotate(90)").style("text-anchor", "start");
				return ax;
			}
			return null;
		}
...
var script$c = defineComponent({
	...
	setup(props) {
		...
		watch(() => [
			Object.assign({}, axisSpace),
			props.direction,
			props.size,
			props.margin,
			props.axis
		], () => {
			if (chart) chart.changeConfig({
		...
		const resizeObserver = new ResizeObserver((entries) => {
			for (const entry of entries) if (axBottomEl.value && entry.target === axBottomEl.value.$el) axisSpace.y = entry.contentRect.height;
			else if (axLeftEl.value && entry.target === axLeftEl.value.$el) axisSpace.x = entry.contentRect.width;
		});
		onMounted(() => {
			if (axBottomEl.value) resizeObserver.observe(axBottomEl.value.$el);
			if (axLeftEl.value) resizeObserver.observe(axLeftEl.value.$el);
		});
		onUnmounted(() => {
			resizeObserver.disconnect();
		});
		function onMouseOut() {
			if (props.config.controlHover === false) return;
			mouse.index = -1;
			mouse.hover = false;
		}
		function onMouseMove(e) {
			...
			const keys = chart.getKeys(0);
			const vals = chart.getData(keys);
			if (chart.config.direction === "horizontal") if (primary.type === "band") {
				const band = primary.bandwidth();
				const delta = mouse.position.x - chart.canvas.x;
				mouse.index = Math.round((delta + band / 2) / band) - 1;
			} else mouse.index = bisectCenter(vals, primary.scale.invert(mouse.position.x));
			else if (chart.config.direction === "vertical") if (primary.type === "band") {
				const band = primary.bandwidth();
				const delta = mouse.position.y - chart.canvas.y;
				mouse.index = Math.round((delta + band / 2) / band) - 1;
			} else mouse.index = bisectCenter(vals, primary.scale.invert(mouse.position.y));
		}
		return {
			axBottomEl,
			axLeftEl,
...
function render$c(_ctx, _cache, $props, $setup, $data, $options) {
	...
	}, [createBaseVNode("g", _hoisted_3$6, [renderSlot(_ctx.$slots, "layers")]), createBaseVNode("g", _hoisted_4$3, [!_ctx.hideX ? (openBlock(), createBlock(_component_axis, {
		key: 0,
		position: "bottom",
		ref: "axBottomEl",
		rotate: _ctx.rotateX,
		isPrimary: _ctx.direction === "horizontal"
	}, null, 8, ["rotate", "isPrimary"])) : createCommentVNode("v-if", true), !_ctx.hideY ? (openBlock(), createBlock(_component_axis, {
		key: 1,
		position: "left",
		ref: "axLeftEl",
		isPrimary: _ctx.direction === "vertical"
	}, null, 8, ["isPrimary"])) : createCommentVNode("v-if", true)])], 40, _hoisted_2$7)), createBaseVNode("div", _hoisted_5$3, [renderSlot(_ctx.$slots, "widgets")])], 512);
}
...
function render$b(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("g", { class: normalizeClass(`layer-${_ctx.type}`) }, [renderSlot(_ctx.$slots, "default")], 2);
}
...
var script$a = defineComponent({
	...
	setup(props) {
		...
		const buildArea = () => {
			...
			if (stacked) return area().curve(lineType(props.type)).x0((p) => p.x + p.width / 2).y0((p) => p.y).x1((p) => p.x + p.width / 2).y1((p) => p.y + p.height);
			if (chart.config.direction === "vertical") return area().curve(lineType(props.type)).y0((p) => p.y).y1((p) => p.y).x0((p) => p.x).x1(() => secondary.scale(0));
...
function render$a(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createBlock(_component_Layer, {
		...
		default: withCtx(() => [createBaseVNode("path", mergeProps(_ctx.toKebabCase(_ctx.getStyle()), { d: _ctx.d }), null, 16, _hoisted_1$9)]),
		_: 1
	}, 8, ["dataKeys"]);
}
...
var script$9 = defineComponent({
	...
	setup(props) {
		...
		function updateLine() {
			...
			if (props.type === "monotone" && chart.config.direction === "vertical") type = monotoneY;
		...
		const getStyle = computed(() => {
			if (is$1(Function, props.lineStyle)) return props.lineStyle;
			return () => Object.assign({
				stroke: "#8884d8",
				strokeWidth: 2
			}, props.lineStyle);
		...
		const getDotStyle = computed(() => {
			if (is$1(Function, props.dotStyle)) return props.dotStyle;
...
function render$9(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createBlock(_component_Layer, {
		...
		default: withCtx(() => [
			...
			!_ctx.hideDot ? (openBlock(), createElementBlock("g", _hoisted_2$6, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.points, (c, i) => {
...
var script$8 = defineComponent({
	...
	setup(props, { emit }) {
		...
		const getStyle = computed(() => {
			if (is$1(Function, props.barStyle)) return props.barStyle;
			return () => props.barStyle;
		});
		const getStyleSelected = computed(() => {
			if (is$1(Function, props.barStyleSelected)) return props.barStyleSelected;
	...
	return openBlock(), createBlock(_component_Layer, {
		...
		default: withCtx(() => [
			...
			(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.bars, (bar, i) => {
				return openBlock(), createElementBlock("rect", mergeProps({ class: "chart-bar" }, _ctx.toKebabCase(_ctx.getStyle({
					...bar.props,
					active: i === _ctx.mouse.index
				})), {
					key: i,
...
function render$7(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createBlock(_component_Layer, {
		...
		default: withCtx(() => [_ctx.arcs.length === 0 ? (openBlock(), createElementBlock("text", _hoisted_1$6, "No Data")) : createCommentVNode("v-if", true), createBaseVNode("g", { transform: _ctx.transform }, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.arcs, (arc, i) => {
...
function render$5(_ctx, _cache, $props, $setup, $data, $options) {
	return _ctx.data.length ? (openBlock(), createElementBlock("g", {
		...
		!_ctx.hideY ? (openBlock(), createElementBlock("g", _hoisted_4$2, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.yLines, (x, i) => {
			return openBlock(), createElementBlock("line", {
				key: i,
				x1: x,
				y1: _ctx.canvas.y,
		...
		createBaseVNode("g", null, [createBaseVNode("line", {
			x1: _ctx.canvas.x,
			y1: _ctx.canvas.y,
			x2: _ctx.canvas.width,
			y2: _ctx.canvas.y
		}, null, 8, _hoisted_6$2), createBaseVNode("line", {
	...
	setup(props) {
		...
		const style = computed(() => {
			if (direction.value === "vertical") return {
				top: position.value.y - .5 + "px",
				left: canvas.value.x + "px"
			};
			else if (direction.value === "horizontal") return {
				top: canvas.value.y + "px",
				left: position.value.x - .5 + "px"
			};
			else return {
				top: canvas.value.y + "px",
				left: canvas.value.x + "px"
...
function render$4(_ctx, _cache, $props, $setup, $data, $options) {
	return _ctx.show ? (openBlock(), createElementBlock("div", {
		...
		ref: "el"
	}, [renderSlot(_ctx.$slots, "default", { payload: _ctx.items }, () => [createBaseVNode("div", { style: normalizeStyle({ borderColor: _ctx.color }) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.items, (item, i) => {
		return openBlock(), createElementBlock("div", {
			key: i,
			style: normalizeStyle({ color: item.color })
		}, [createBaseVNode("b", null, toDisplayString(item.label), 1), createTextVNode(": " + toDisplayString(item.valueFormatted), 1)], 4);
	}), 128))], 4)])], 4), !_ctx.hideLine ? (openBlock(), createElementBlock("div", _hoisted_1$3, [_ctx.direction === "horizontal" ? (openBlock(), createElementBlock("div", {
		key: 0,
		class: "v-tooltip-line-vertical",
		style: normalizeStyle({ height: _ctx.canvas.height + "px" })
	}, null, 4)) : (openBlock(), createElementBlock("div", {
		key: 1,
		class: "v-tooltip-line-horizontal",
		style: normalizeStyle({ width: _ctx.canvas.width - 40 + "px" })
	}, null, 4))])) : createCommentVNode("v-if", true)], 4)) : createCommentVNode("v-if", true);
}
...
var script$2 = defineComponent({
	...
	setup(props) {
		...
		const canvas = ref({});
		watch(chart.updates, () => {
			if (props.value !== void 0) y.value = chart.scales.secondary.scale(props.value);
			canvas.value = chart.canvas;
			direction.value = chart.config.direction;
		}, { immediate: true });
		watch(() => props.value, () => {
...
function render$2(_ctx, _cache, $props, $setup, $data, $options) {
	return _ctx.direction === "horizontal" ? (openBlock(), createElementBlock("g", _hoisted_1$2, [createBaseVNode("text", {
		x: _ctx.canvas.x + 5,
		y: _ctx.y - 5,
		"text-anchor": "start",
		fill: _ctx.color
	}, toDisplayString(_ctx.label), 9, _hoisted_2$2), createBaseVNode("line", {
		x1: _ctx.canvas.x,
		y1: _ctx.y,
		x2: _ctx.canvas.width,
		y2: _ctx.y,
		...
		"stroke-dasharray": _ctx.strokeDasharray
	}, null, 8, _hoisted_3$1)])) : (openBlock(), createElementBlock("g", _hoisted_4$1, [createBaseVNode("text", {
		x: _ctx.y + 10,
		y: _ctx.canvas.y + 5,
		"text-anchor": "start",
		fill: _ctx.color,
		style: { "writing-mode": "tb" }
	}, toDisplayString(_ctx.label), 9, _hoisted_5$1), createBaseVNode("line", {
		x1: _ctx.y,
		y1: _ctx.canvas.y,
		x2: _ctx.y,
		y2: _ctx.canvas.height,
...
var script$1 = defineComponent({
	...
	setup(props) {
		...
		function getScales() {
			...
			return {
				x: linear().domain(values).range([0, 100]).nice(),
				y: sequential().domain(values).interpolator(props.colorFn())
			};
		}
		function buildRectangles() {
			const { y } = getScales();
			return buildTree(data.value).leaves().map((d, idx) => {
				return {
					idx,
		...
		function buildTree(data) {
			...
			const tree = stratify().id((d) => d.name).parentId((d) => d.parent)([{ name: "root" }].concat(data.map((x) => {
				return {
		...
		const getStyle = (r) => {
			...
			if (props.rcStyle && typeof props.rcStyle === "function") return Object.assign(Object.assign({}, style), props.rcStyle(r));
			return style;
		};
...
function render$1(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("div", _hoisted_1$1, [(openBlock(), createElementBlock("svg", {
		...
		viewBox: `0 0 ${_ctx.size.width} ${_ctx.size.height}`,
		ref: "el"
	}, [createBaseVNode("g", _hoisted_3, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.rectangles, (r, i) => {
		return openBlock(), createElementBlock("g", {
			...
			width: r.width,
			height: r.height
		}), null, 16, _hoisted_5), renderSlot(_ctx.$slots, "text", {
			r,
			selected: _ctx.selected === i
		}, () => [createBaseVNode("text", _hoisted_6, toDisplayString(r.data.name), 1)])])], 42, _hoisted_4);
	}), 128))])], 8, _hoisted_2$1)), renderSlot(_ctx.$slots, "widgets")]);
}
	...
	setup() {
		...
		watch(mouse, () => {
			if (chart.bandScale) {
				...
				if (x) if (chart.config.direction === "horizontal") bar.value = {
					x: x - width / 2,
					y: 0,
					width,
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
	return _ctx.mouse.hover ? (openBlock(), createElementBlock("g", _hoisted_1, [createBaseVNode("rect", mergeProps({ class: "hover-bar" }, _ctx.barStyle, {
		x: _ctx.bar.x,
		y: _ctx.bar.y,

...
*/
function setInsertionState(parent, anchor, logicalIndex = anchor === 0 ? 0 : void 0) {
...
function enterHydration(node) {
	const prevHydrationEnabled = isHydratingEnabled$1;
...
function queuePendingSlotContentAnchor(anchor) {
	...
	if (state && state.pending) (state.pendingAnchors || (state.pendingAnchors = [])).push(anchor);
}
...
function resolveDynamicAnchor(frag, isEmpty) {
	...
	if (isEmpty) {
		if (isPendingSlotContent()) {
			...
			return {
				kind: "pending",
				parent: /* @__PURE__ */ parentNode(currentHydrationNode || slotEnd),
				slotEnd
			};
		}
	...
	if (frag.isSlot || anchorLabel === "if" && isArray(frag.nodes) && frag.nodes.length > 1) {
			...
			case "pending": {
				...
				queuePendingSlotContentAnchor({
					onContent: () => {
						...
						const parentNode$6 = /* @__PURE__ */ parentNode(anchor);
						if (parentNode$6) parentNode$6.insertBefore(createRuntimeAnchor(), anchor);
					},
					onFallback: () => {
						createRuntimeAnchor();
					}
...
function createSlot(name = "default", rawProps, fallback, flags = 0) {
	...
	if (/* @__PURE__ */ isRef(rawSlots._) && isInteropEnabled) {
		...
		if (isHydrating$1) fragment.forwarded = currentSlotOwner != null && currentSlotOwner !== currentInstance;
...
var DynamicFragment = class extends RenderContextFragment {
	constructor(anchorLabel, keyed = false, locate = true, trackSlotBoundary = false, onInvalid) {
		super(EMPTY_BLOCK$1);
		this.__df = true;
	...
	update(render, key = render, noScope = false) {
		...
		if (wasMounted) {
			...
			remove(this.nodes, parent || void 0);
		}
...
var SlotFragment = class extends DynamicFragment {
	...
	updateSlot(render, fallback, key = render || fallback) {
		...
		try {
			...
			if (isHydrating$1) if (this.forwarded && !fallback) {
				...
				if (contentValid) {
					...
					} else hydrateDynamicFragmentAnchor(this, !isValidBlock(this.content));
				} else {
					var _this$anchorLabel;
					const anchor = this.anchor = markHydrationAnchor(/* @__PURE__ */ createComment((_this$anchorLabel = this.anchorLabel) !== null && _this$anchorLabel !== void 0 ? _this$anchorLabel : ""));
					const slotEnd = getCurrentSlotEndAnchor();
...
function insert(block, parent, anchor = null, parentSuspense) {
	...
	if (isVaporComponent(block)) {
		...
		anchor = anchor === 0 ? parent.$fc || /* @__PURE__ */ _child(parent) : anchor;
		for (const b of block) insert(b, parent, anchor, parentSuspense);
...
function findBlockBoundary(block) {
	...
	if (nextNode && isComment(nextNode, "]") && isFragmentBlock(block) && !isComment(lastChild, "]") && !(lastChild.nodeType === 3 && !lastChild.data)) nextNode = nextNode.nextSibling;
	return {
		parentNode,
		nextNode
	};
...
function mountComponent(instance, parent, anchor) {
	if (isSuspenseEnabled && instance.suspense && instance.asyncDep && !instance.asyncResolved) {
		if (instance.asyncDepRegistered) {
			if (instance.block) insert(instance.block, parent, anchor);
		...
		const component = instance.type;
		const hydrationParent = hydrating ? parent : void 0;
		const hydrationAnchor = hydrating ? anchor : void 0;
		instance.suspense.registerDep(instance, (setupResult) => {
			...
			try {
				...
				if (hydrating) instance.deferredHydrationBoundary = void 0;
...
function activate$1(instance, parentNode, anchor, parentSuspense = instance.suspense) {
	move(instance.block, parentNode, anchor, 0, instance, parentSuspense);
	queuePostRenderEffect(() => {
		instance.isDeactivated = false;
		if (instance.a) invokeArrayFns(instance.a);
	}, void 0, parentSuspense);
...
function deactivate$1(instance, container, parentSuspense = instance.suspense) {
	...
	invalidateMount(instance.m);
	invalidateMount(instance.a);
	move(instance.block, container, null, 1, instance, parentSuspense);
	queuePostRenderEffect(() => {
		if (instance.da) invokeArrayFns(instance.da);
		instance.isDeactivated = true;
	}, void 0, parentSuspense);
...
function getInteropTransitionElement(vnode) {
	if (vnode.component) return getInteropTransitionElement(vnode.component.subTree);
...
var vaporInteropImpl = {
	...
	unmount(vnode, doRemove, parentSuspense) {
		...
		if (instance) {
			...
			if (instance.block) {
				...
				const adoptedNode = vnode.el && vnode.el !== anchor ? vnode.el : null;
				unmountComponent(instance, void 0, parentSuspense);
				if (doRemove && container && adoptedNode) {
					let cur = adoptedNode;
					while (cur && cur !== anchor) {
						const nextNode = cur.nextSibling;
						if (cur.parentNode === container) remove(cur, container);
						cur = nextNode;
					}
				}
			}
	...
	*/
	...
	move(vnode, container, anchor, moveType, parentSuspense) {
		if (vnode.el && vnode.el !== vnode.anchor && isComment(vnode.el, "[")) move(vnode.el, container, anchor, moveType, void 0, parentSuspense);
		move(vnode.vb || vnode.component, container, anchor, moveType, void 0, parentSuspense);
...
function mountVNode(internals, vnode, parentComponent) {
	...
	frag.insert = (parentNode, anchor, parentSuspense, transition) => {
		if (isHydrating$1) return;
		const operationSuspense = parentSuspense === void 0 ? suspense : parentSuspense;
		if (vnode.shapeFlag & 512) {
			if (vnode.type.__vapor) activate$1(vnode.component, parentNode, anchor, operationSuspense);
			...
			if (!isMounted) {
				...
				onScopeDispose(unmount, true);
				isMounted = true;
			} else internals.m(vnode, parentNode, anchor, 2, parentComponent, operationSuspense);
			simpleSetCurrentInstance(prev);
		}
		syncNodes();
...
function createVDOMComponent(internals, component, parentComponent, rawProps, rawSlots, once) {
	...
	const unmount = (parentNode, transition) => {
		...
		if (vnode.shapeFlag & 256) {
			deactivate(vnode, keepAliveCtx.getStorageContainer(), internals, parentComponent, null);
			return;
		}
		isUnmounted = true;
		isMounted = false;
...
function renderVDOMSlot(internals, slotsRef, name, props, parentComponent, fallback, once, slotRoot) {
	...
	if (slotRoot) trackSlotBoundaryDirtying(frag, cleanupInvalidContent);
	...
	const removeRenderedContent = (rendered, parentNode) => {
		...
		if (isVNode(rendered)) internals.um(rendered, parentComponent, null, !contentDetached && !!parentNode);
	...
	frag.insert = (parentNode, anchor, parentSuspense) => {
		...
		if (!isMounted) {
			...
			if (isVNode(contentState.rendered)) internals.m(contentState.rendered, parentNode, anchor, 2, parentComponent, suspense);
	...
	frag.remove = (parentNode) => {
		...
		scope.stop();
		disposed = true;
		if (contentState.rendered) removeRenderedContent(contentState.rendered, parentNode);
				...
				runWithFragmentCtxOnly(frag, () => withSlotBoundary(boundary, () => {
					...
					if (isHydrating$1) {
						...
						if (isVNode(hydratedContent)) {
							...
							trackSlotVNodeUpdatesWithRefresh(hydratedContent, refreshSlotVNode, notifyBeforeUpdate);
					...
					if (isVNode(slotContent)) {
						...
						trackSlotVNodeUpdatesWithRefresh(slotContent, refreshSlotVNode, notifyBeforeUpdate);
...
var VaporTransitionGroup = /* @__PURE__ */ decorate(/* @__PURE__ */ defineVaporComponent({
	...
	setup(props, { slots, expose }) {
		expose();
		if (!isTransitionEnabled) registerTransitionHooks(applyTransitionHooksImpl, () => false, () => false);
		const instance = currentInstance;
		const state = useTransitionState();
...
function applyGroupTransitionHooks(block, props, state, instance, updateHooks) {
	...
	for (let i = 0; i < children.length; i++) {
		const child = children[i];
		if (isValidTransitionBlock(child)) if (child.$key != null) child.$transition = resolveTransitionHooks(child, props, state, instance);
...
//#endregion
export { setBlockText as $, isProxy as $a, resolvePropValue as $i, createCanSetSetupRefChecker as $n, isMismatchAllowed as $r, unsafeToTrustedHTML as $t, createVaporApp as A, validateProps as Aa, onUnmounted as Ai, MoveType as An, getFunctionalFallthrough as Ar, createApp as At, insert as B, withDefaults as Ba, pushWarningContext as Bi, baseEmit as Bn, hydrateOnVisible as Br, hydrate as Bt, createInvoker as C, useInstanceOption as Ca, onBeforeUpdate as Ci, Comment$1 as Cn, extend as Co, ensureValidVNode as Cr, TransitionGroup as Ct, createSlot as D, useTemplateRef as Da, onRenderTracked as Di, Fragment as Dn, toDisplayString as Do, getAttributeMismatch as Dr, baseApplyTranslation as Dt, createSelector as E, useSlots as Ea, onMounted as Ei, ErrorTypeStrings as En, normalizeStyle as Eo, flushOnAppMount as Er, VueElementBase as Et, defineVaporSSRCustomElement as F, watch as Fa, performTransitionLeave as Fi, Teleport as Fn, handleError as Fr, ensureRenderer as Ft, on as G, ReactiveEffect as Ga, registerRuntimeCompiler as Gi, checkTransitionMode as Gn, invokeDirectiveHook as Gr, patchProp as Gt, isVaporComponent as H, withMemo as Ha, queuePostFlushCb as Hi, baseResolveTransitionHooks as Hn, initFeatureFlags as Hr, nodeOps as Ht, delegate as I, watchEffect as Ia, popScopeId as Ii, Text$1 as In, hasInjectionContext as Ir, forceReflow as It, remove as J, customRef as Ja, resetShapeFlag as Ji, computed as Jn, isHydrating as Jr, resolveTransitionProps as Jt, onBinding as K, TrackOpTypes as Ka, renderList as Ki, cloneVNode as Kn, isAsyncWrapper as Kr, patchStyle as Kt, delegateEvents as L, watchPostEffect as La, popWarningContext as Li, VaporSlot as Ln, hydrateOnIdle as Lr, getValue as Lt, defineVaporAsyncComponent as M, warn as Ma, openBlock as Mi, SchedulerJobFlags as Mn, getTransitionRawChildren as Mr, defineCustomElement as Mt, defineVaporComponent as N, warnExtraneousAttributes as Na, performAsyncHydrate as Ni, Static as Nn, guardReactiveProps as Nr, defineSSRCustomElement as Nt, createTemplateRefSetter as O, useTransitionState as Oa, onRenderTriggered as Oi, KeepAlive as On, toHandlerKey as Oo, getComponentName as Or, baseUseCssVars as Ot, defineVaporCustomElement as P, warnPropMismatch as Pa, performTransitionEnter as Pi, Suspense as Pn, h as Pr, ensureHydrationRenderer as Pt, setBlockKey as Q, getCurrentWatcher as Qa, resolveFilter as Qi, createBlock as Qn, isMemoSame as Qr, svgNS as Qt, getDefaultValue as R, watchSyncEffect as Ra, provide as Ri, activate as Rn, hydrateOnInteraction as Rr, handleMovedChildren as Rt, createIf as S, useId as Sa, onBeforeUnmount as Si, BaseTransitionPropsValidators as Sn, capitalize as So, endMeasure as Sr, Transition as St, createPlainElement as T, useSSRContext as Ta, onErrorCaptured as Ti, ErrorCodes as Tn, normalizeProps as To, expose as Tr, VueElement as Tt, next as U, withScopeId as Ua, queuePostRenderEffect as Ui, callWithAsyncErrorHandling as Un, inject as Ur, normalizeContainer as Ut, isFragment as V, withDirectives as Va, queueJob as Vi, baseNormalizePropsOptions as Vn, initCustomFormatter as Vr, initDirectivesForSSR as Vt, nthChild as W, EffectScope as Wa, registerHMR as Wi, callWithErrorHandling as Wn, invalidateMount as Wr, parseEventName as Wt, setAttr as X, effectScope as Xa, resolveDirective as Xi, createAsyncComponentContext as Xn, isKeepAlive as Xr, shouldSetAsProp as Xt, renderEffect as Y, effect as Ya, resolveComponent as Yi, createAppAPI as Yn, isHydratingEnabled as Yr, setVarsOnNode as Yt, setBlockHtml as Z, getCurrentScope as Za, resolveDynamicComponent as Zi, createBaseVNode as Zn, isMapEqual as Zr, shouldSetAsPropForVueCE as Zt, createComponent as _, toStyleMap as _a, nextUid as _i, vShowOriginalDisplay as _n, toRefs as _o, defineOptions as _r, vaporInteropPlugin as _t, VaporKeepAlive as a, setCurrentRenderingInstance as aa, isTemplateRefKey as ai, vModelCheckboxInit as an, onScopeDispose as ao, createRenderer as ar, setElementText as at, createFor as b, useAsyncComponentState as ba, onActivated as bi, xlinkNS as bn, unref as bo, devtools as br, withVaporKeys as bt, VaporTransitionGroup as c, setRef as ca, knownTemplateRefs as ci, vModelRadio as cn, reactive as co, createTextVNode as cr, setProp as ct, applyRadioModel as d, simpleSetCurrentInstance as da, markAsyncBoundary as di, vModelSetSelected as dn, shallowReactive as do, deactivate as dr, setTemplateRefBinding as dt, resolveTarget as ea, isRuntimeOnly as ei, useCssModule as en, isReactive as eo, createCommentVNode as er, setClass as et, applySelectModel as f, ssrContextKey as fa, matches as fi, vModelText as fn, shallowReadonly as fo, defineAsyncComponent as fr, setText as ft, createAssetComponent as g, toHandlers as ga, nextTick as gi, vShowHidden as gn, toRef as go, defineModel as gr, useVaporCssVars as gt, child as h, toClassSet as ha, mergeProps as hi, vShow as hn, toRaw as ho, defineExpose as hr, txt as ht, VaporFragment as i, setCurrentInstance as ia, isTemplateNode as ii, vModelCheckbox as in, markRaw as io, createPropsRestProxy as ir, setDynamicProps as it, createVaporSSRApp as j, version as ja, onUpdated as ji, NULL_DYNAMIC_COMPONENT as jn, getInheritedScopeIds as jr, createSSRApp as jt, createTextNode as k, validateComponentName as ka, onServerPrefetch as ki, MismatchTypes as kn, getCurrentInstance as kr, callPendingCbs as kt, applyCheckboxModel as l, setTransitionHooks as la, leaveCbKey as li, vModelSelect as ln, readonly as lo, createVNode as lr, setStaticTemplateRef as lt, applyVShow as m, startMeasure as ma, mergeModels as mi, vModelTextUpdate as mn, stop as mo, defineEmits as mr, template as mt, DynamicFragment as n, restoreCurrentInstance as na, isTeleportDeferred as ni, useHost as nn, isRef as no, createHydrationRenderer as nr, setDOMProp as nt, VaporTeleport as o, setDevtoolsHook as oa, isVNode as oi, vModelCheckboxUpdate as on, onWatcherCleanup as oo, createSlots as or, setHtml as ot, applyTextModel as p, ssrUtils as pa, mergeDefaults as pi, vModelTextInit as pn, shallowRef as po, defineComponent as pr, setValue as pt, prepend as q, TriggerOpTypes as qa, renderSlot as qi, compatUtils as qn, isEmitListener as qr, render as qt, VaporElement as r, setBlockTracking as ra, isTeleportDisabled as ri, useShadowRoot as rn, isShallow as ro, createInternalObject as rr, setDynamicEvents as rt, VaporTransition as s, setIsHydratingEnabled$1 as sa, isValidHtmlOrSvgAttribute as si, vModelDynamic as sn, proxyRefs as so, createStaticVNode as sr, setInsertionState as st, compile as t, resolveTransitionHooks$1 as ta, isSetEqual as ti, useCssVars as tn, isReadonly as to, createElementBlock as tr, setClassName as tt, applyDynamicModel as u, shouldUpdateComponent as ua, logMismatchError as ui, vModelSelectInit as un, ref as uo, currentInstance as ur, setStyle as ut, createComponentWithFallback as v, transformVNodeArgs as va, normalizeRef as vi, withKeys as vn, toValue as vo, defineProps as vr, withAsyncContext as vt, createKeyedFragment as w, useModel as wa, onDeactivated as wi, DeprecationTypes as wn, normalizeClass as wo, ensureVaporSlotFallback as wr, TransitionPropsValidators as wt, createForSlots as x, useAttrs as xa, onBeforeMount as xi, BaseTransition as xn, camelize as xo, devtoolsComponentAdded as xr, withVaporModifiers as xt, createDynamicComponent as y, unregisterHMR as ya, normalizeVNode as yi, withModifiers as yn, triggerRef as yo, defineSlots as yr, withVaporDirectives as yt, getRestElement as z, withCtx as za, pushScopeId as zi, assertNumber as zn, hydrateOnMediaQuery as zr, hasCSSTransform as zt };
